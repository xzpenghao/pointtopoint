package com.ztgeo.pointtopoint.handle.entity;

/**
 * 农用地使用权
 */
public class NYDSYQ {
    private String id;
    private String cxrid;
    private String bdcdyh;//不动产权证号
    private String zl;//坐落
    private String cbmj;//承包面积
    private String cbqssj;//承包起始时间
    private String cbjssj;//承包结束时间
    private String bdcqzh;//不动产权证号
    private String djjg;//登记机构
    private String tdsyqxz;//土地使用权性质
    private String djsj;//登记时间
    private String gyfs;//共有方式
    private String gyr;//共有人 新增
    private String gyqk;//共有情况 新增
    private String qszt;//权属状态 新增
    private String ywh;//业务号 新增
    private String qlrdh;//权利人电话 新增
    private String qxdm;//区县代码 新增
    private String syttlx;//水域滩涂类型 新增
    private String qllx;//权力类型 新增
    private String sfdy;//是否抵押 新增
    private String sfcf;//是否查封 新增
    private String yzyfs;//养殖业方式 新增
    private String cyzl;//草原质量 新增
    private String sszcl;//适宜载畜量
    private String fbfdm;//发包方代码
    private String fbfmc;//发包方名称

    public NYDSYQ() {
        super();
        // TODO Auto-generated constructor stub
    }

    public NYDSYQ(String id, String cxrid, String bdcdyh, String zl, String cbmj, String cbqssj, String cbjssj, String bdcqzh, String djjg, String tdsyqxz, String djsj, String gyfs, String gyr, String gyqk, String qszt, String ywh, String qlrdh, String qxdm, String syttlx, String qllx, String sfdy, String sfcf, String yzyfs, String cyzl, String sszcl, String fbfdm, String fbfmc) {
        this.id = id;
        this.cxrid = cxrid;
        this.bdcdyh = bdcdyh;
        this.zl = zl;
        this.cbmj = cbmj;
        this.cbqssj = cbqssj;
        this.cbjssj = cbjssj;
        this.bdcqzh = bdcqzh;
        this.djjg = djjg;
        this.tdsyqxz = tdsyqxz;
        this.djsj = djsj;
        this.gyfs = gyfs;
        this.gyr = gyr;
        this.gyqk = gyqk;
        this.qszt = qszt;
        this.ywh = ywh;
        this.qlrdh = qlrdh;
        this.qxdm = qxdm;
        this.syttlx = syttlx;
        this.qllx = qllx;
        this.sfdy = sfdy;
        this.sfcf = sfcf;
        this.yzyfs = yzyfs;
        this.cyzl = cyzl;
        this.sszcl = sszcl;
        this.fbfdm = fbfdm;
        this.fbfmc = fbfmc;
    }

    public String getCxrid() {
        return cxrid;
    }

    public void setCxrid(String cxrid) {
        this.cxrid = cxrid;
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getBdcdyh() {
        return bdcdyh;
    }

    public void setBdcdyh(String bdcdyh) {
        this.bdcdyh = bdcdyh;
    }

    public String getZl() {
        return zl;
    }

    public void setZl(String zl) {
        this.zl = zl;
    }

    public String getCbmj() {
        return cbmj;
    }

    public void setCbmj(String cbmj) {
        this.cbmj = cbmj;
    }

    public String getCbqssj() {
        return cbqssj;
    }

    public void setCbqssj(String cbqssj) {
        this.cbqssj = cbqssj;
    }

    public String getCbjssj() {
        return cbjssj;
    }

    public void setCbjssj(String cbjssj) {
        this.cbjssj = cbjssj;
    }

    public String getBdcqzh() {
        return bdcqzh;
    }

    public void setBdcqzh(String bdcqzh) {
        this.bdcqzh = bdcqzh;
    }

    public String getDjjg() {
        return djjg;
    }

    public void setDjjg(String djjg) {
        this.djjg = djjg;
    }

    public String getTdsyqxz() {
        return tdsyqxz;
    }

    public void setTdsyqxz(String tdsyqxz) {
        this.tdsyqxz = tdsyqxz;
    }

    public String getDjsj() {
        return djsj;
    }

    public void setDjsj(String djsj) {
        this.djsj = djsj;
    }

    public String getGyfs() {
        return gyfs;
    }

    public void setGyfs(String gyfs) {
        this.gyfs = gyfs;
    }

    public String getGyr() {
        return gyr;
    }

    public void setGyr(String gyr) {
        this.gyr = gyr;
    }

    public String getGyqk() {
        return gyqk;
    }

    public void setGyqk(String gyqk) {
        this.gyqk = gyqk;
    }

    public String getQszt() {
        return qszt;
    }

    public void setQszt(String qszt) {
        this.qszt = qszt;
    }

    public String getYwh() {
        return ywh;
    }

    public void setYwh(String ywh) {
        this.ywh = ywh;
    }

    public String getQlrdh() {
        return qlrdh;
    }

    public void setQlrdh(String qlrdh) {
        this.qlrdh = qlrdh;
    }

    public String getQxdm() {
        return qxdm;
    }

    public void setQxdm(String qxdm) {
        this.qxdm = qxdm;
    }

    public String getSyttlx() {
        return syttlx;
    }

    public void setSyttlx(String syttlx) {
        this.syttlx = syttlx;
    }

    public String getQllx() {
        return qllx;
    }

    public void setQllx(String qllx) {
        this.qllx = qllx;
    }

    public String getSfdy() {
        return sfdy;
    }

    public void setSfdy(String sfdy) {
        this.sfdy = sfdy;
    }

    public String getSfcf() {
        return sfcf;
    }

    public void setSfcf(String sfcf) {
        this.sfcf = sfcf;
    }

    public String getYzyfs() {
        return yzyfs;
    }

    public void setYzyfs(String yzyfs) {
        this.yzyfs = yzyfs;
    }

    public String getCyzl() {
        return cyzl;
    }

    public void setCyzl(String cyzl) {
        this.cyzl = cyzl;
    }

    public String getSszcl() {
        return sszcl;
    }

    public void setSszcl(String sszcl) {
        this.sszcl = sszcl;
    }

    public String getFbfdm() {
        return fbfdm;
    }

    public void setFbfdm(String fbfdm) {
        this.fbfdm = fbfdm;
    }

    public String getFbfmc() {
        return fbfmc;
    }

    public void setFbfmc(String fbfmc) {
        this.fbfmc = fbfmc;
    }

    @Override
    public String toString() {
        return "DirectNYDSYQ{" +
                "bdcdyh='" + bdcdyh + '\'' +
                ", zl='" + zl + '\'' +
                ", cbmj='" + cbmj + '\'' +
                ", cbqssj='" + cbqssj + '\'' +
                ", cbjssj='" + cbjssj + '\'' +
                ", bdcqzh='" + bdcqzh + '\'' +
                ", djjg='" + djjg + '\'' +
                ", tdsyqxz='" + tdsyqxz + '\'' +
                ", djsj='" + djsj + '\'' +
                ", gyfs='" + gyfs + '\'' +
                ", gyr='" + gyr + '\'' +
                ", gyqk='" + gyqk + '\'' +
                ", qszt='" + qszt + '\'' +
                ", ywh='" + ywh + '\'' +
                ", qlrdh='" + qlrdh + '\'' +
                ", qxdm='" + qxdm + '\'' +
                ", syttlx='" + syttlx + '\'' +
                ", qllx='" + qllx + '\'' +
                ", sfdy='" + sfdy + '\'' +
                ", sfcf='" + sfcf + '\'' +
                ", yzyfs='" + yzyfs + '\'' +
                ", cyzl='" + cyzl + '\'' +
                ", sszcl='" + sszcl + '\'' +
                ", fbfdm='" + fbfdm + '\'' +
                ", fbfmc='" + fbfmc + '\'' +
                '}';
    }
}
