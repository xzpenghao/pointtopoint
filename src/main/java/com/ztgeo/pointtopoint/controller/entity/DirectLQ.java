package com.ztgeo.pointtopoint.controller.entity;

/**
 * 林权
 */
public class DirectLQ {
	private String bdcdyh;//不动产单元号
	private String zl;//坐落
	private String syqmj;//使用权面积
	private String ldsyqssj;//林地使用起始时间
	private String ldsyjssj;//林地使用结束时间
	private String bdcqzh;//不动产权证号
	private String djjg;//登记机构
	private String ldsyqxz;//林地使用权性质
	private String djsj;//登记时间
	private String gyfs;//共有方式
	private String gyr;//共有人 新增
	private String gyqk;//共有情况 新增
	private String qszt;//权属状态 新增
	private String ywh;//业务号 新增
	private String qlrdh;//权利人电话 新增
	private String qxdm;//区县代码 新增
	private String lz;//林种 新增
	private String qllx;//权力类型 新增
	private String sfdy;//是否抵押 新增
	private String sfcf;//是否查封 新增
	private String sllmsyqr1;//森林、林木所有权人 新增
	private String sllmsyqr2;//森林、林木所有权人 新增
	
	public DirectLQ() {
		super();
		// TODO Auto-generated constructor stub
	}

	public DirectLQ(String bdcdyh, String zl, String syqmj, String ldsyqssj, String ldsyjssj, String bdcqzh, String djjg, String ldsyqxz, String djsj, String gyfs, String gyr, String gyqk, String qszt, String ywh, String qlrdh, String qxdm, String lz, String qllx, String sfdy, String sfcf, String sllmsyqr1, String sllmsyqr2) {
		this.bdcdyh = bdcdyh;
		this.zl = zl;
		this.syqmj = syqmj;
		this.ldsyqssj = ldsyqssj;
		this.ldsyjssj = ldsyjssj;
		this.bdcqzh = bdcqzh;
		this.djjg = djjg;
		this.ldsyqxz = ldsyqxz;
		this.djsj = djsj;
		this.gyfs = gyfs;
		this.gyr = gyr;
		this.gyqk = gyqk;
		this.qszt = qszt;
		this.ywh = ywh;
		this.qlrdh = qlrdh;
		this.qxdm = qxdm;
		this.lz = lz;
		this.qllx = qllx;
		this.sfdy = sfdy;
		this.sfcf = sfcf;
		this.sllmsyqr1 = sllmsyqr1;
		this.sllmsyqr2 = sllmsyqr2;
	}

	public String getBdcdyh() {
		return bdcdyh;
	}

	public void setBdcdyh(String bdcdyh) {
		this.bdcdyh = bdcdyh;
	}

	public String getZl() {
		return zl;
	}

	public void setZl(String zl) {
		this.zl = zl;
	}

	public String getSyqmj() {
		return syqmj;
	}

	public void setSyqmj(String syqmj) {
		this.syqmj = syqmj;
	}

	public String getLdsyqssj() {
		return ldsyqssj;
	}

	public void setLdsyqssj(String ldsyqssj) {
		this.ldsyqssj = ldsyqssj;
	}

	public String getLdsyjssj() {
		return ldsyjssj;
	}

	public void setLdsyjssj(String ldsyjssj) {
		this.ldsyjssj = ldsyjssj;
	}

	public String getBdcqzh() {
		return bdcqzh;
	}

	public void setBdcqzh(String bdcqzh) {
		this.bdcqzh = bdcqzh;
	}

	public String getDjjg() {
		return djjg;
	}

	public void setDjjg(String djjg) {
		this.djjg = djjg;
	}

	public String getLdsyqxz() {
		return ldsyqxz;
	}

	public void setLdsyqxz(String ldsyqxz) {
		this.ldsyqxz = ldsyqxz;
	}

	public String getDjsj() {
		return djsj;
	}

	public void setDjsj(String djsj) {
		this.djsj = djsj;
	}

	public String getGyfs() {
		return gyfs;
	}

	public void setGyfs(String gyfs) {
		this.gyfs = gyfs;
	}

	public String getGyr() {
		return gyr;
	}

	public void setGyr(String gyr) {
		this.gyr = gyr;
	}

	public String getGyqk() {
		return gyqk;
	}

	public void setGyqk(String gyqk) {
		this.gyqk = gyqk;
	}

	public String getQszt() {
		return qszt;
	}

	public void setQszt(String qszt) {
		this.qszt = qszt;
	}

	public String getYwh() {
		return ywh;
	}

	public void setYwh(String ywh) {
		this.ywh = ywh;
	}

	public String getQlrdh() {
		return qlrdh;
	}

	public void setQlrdh(String qlrdh) {
		this.qlrdh = qlrdh;
	}

	public String getQxdm() {
		return qxdm;
	}

	public void setQxdm(String qxdm) {
		this.qxdm = qxdm;
	}

	public String getLz() {
		return lz;
	}

	public void setLz(String lz) {
		this.lz = lz;
	}

	public String getQllx() {
		return qllx;
	}

	public void setQllx(String qllx) {
		this.qllx = qllx;
	}

	public String getSfdy() {
		return sfdy;
	}

	public void setSfdy(String sfdy) {
		this.sfdy = sfdy;
	}

	public String getSfcf() {
		return sfcf;
	}

	public void setSfcf(String sfcf) {
		this.sfcf = sfcf;
	}

	public String getSllmsyqr1() {
		return sllmsyqr1;
	}

	public void setSllmsyqr1(String sllmsyqr1) {
		this.sllmsyqr1 = sllmsyqr1;
	}

	public String getSllmsyqr2() {
		return sllmsyqr2;
	}

	public void setSllmsyqr2(String sllmsyqr2) {
		this.sllmsyqr2 = sllmsyqr2;
	}

	@Override
	public String toString() {
		return "DirectLQ{" +
				"bdcdyh='" + bdcdyh + '\'' +
				", zl='" + zl + '\'' +
				", syqmj='" + syqmj + '\'' +
				", ldsyqssj='" + ldsyqssj + '\'' +
				", ldsyjssj='" + ldsyjssj + '\'' +
				", bdcqzh='" + bdcqzh + '\'' +
				", djjg='" + djjg + '\'' +
				", ldsyqxz='" + ldsyqxz + '\'' +
				", djsj='" + djsj + '\'' +
				", gyfs='" + gyfs + '\'' +
				", gyr='" + gyr + '\'' +
				", gyqk='" + gyqk + '\'' +
				", qszt='" + qszt + '\'' +
				", ywh='" + ywh + '\'' +
				", qlrdh='" + qlrdh + '\'' +
				", qxdm='" + qxdm + '\'' +
				", lz='" + lz + '\'' +
				", qllx='" + qllx + '\'' +
				", sfdy='" + sfdy + '\'' +
				", sfcf='" + sfcf + '\'' +
				", sllmsyqr1='" + sllmsyqr1 + '\'' +
				", sllmsyqr2='" + sllmsyqr2 + '\'' +
				'}';
	}
}
